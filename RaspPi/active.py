...
def accelerationcalc(sense):
    acceleration = sense.get_accelerometer_raw()
    x = abs(acceleration["x"])
    y = abs(acceleration["y"])
    z = abs(acceleration["z"])
    totalacceleration = np.sqrt(x**2+y**2+z**2)
    return totalacceleration

def calibrate(sense):
    calibrationbuffer = []
    for i in range(50):
        totalacceleration = accelerationcalc(sense)
        calibrationbuffer.append(totalacceleration)
        time.sleep(0.1)
    calibrationthreshold = np.mean(calibrationbuffer)+4*np.std(calibrationbuffer)
    return calibrationthreshold
    
def stepcounter(sense, calibrationthreshold):
    count = 0
    totalacceleration = accelerationcalc(sense)
    if totalacceleration > calibrationthreshold:
        count += 1
    time.sleep(0.1)
    return int(count)
