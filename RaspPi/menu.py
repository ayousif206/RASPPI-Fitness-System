...
def activepaths(username, path, ontopalertlabel, gobutton, endbutton, submitbutton, alertlabel2, missingweightinfolabel, missingweightinfoentry, missingweightinfobutton, atspeedlabel, avgspeedlabel, distancelabel, caloriesburnedlabel, stepstakenlabel, elapsedtimelabel, frame, root):
    ...
    if path == 1:
        ...
        if readytostart1 and readytostart2:
            ...
            sense = SenseHat()
            ontopalertlabel.config(text="YOU HAVE 10 SECONDS,\n UNPLUG KEYBOARD AND MOUSE (HDMI IF YOU WANT TO),\nPUT PI IN POCKET,\nTHEN STAND STILL.")
            ontopalertlabel.place(relx=0.5, rely=0.5, anchor=tk.CENTER)
            root.update_idletasks()
            time.sleep(10)
            ontopalertlabel.config(text="CALIBRATING..")
            root.update_idletasks()
            calibrationthreshold = active.calibrate(sense)
            ontopalertlabel.config(text="DONE, YOU MAY BEGIN ACTIVITY!")
            root.update_idletasks()
            time.sleep(1)
            ontopalertlabel.place_forget()
            root.update_idletasks()
            exercising = True
            starttime = time.time()
            def stepdetection():
                ...
                if exercising:
                    numberstepstaken += active.stepcounter(sense, calibrationthreshold)
                    root.after(10, stepdetection)
            def activitytracker():
                if exercising:
